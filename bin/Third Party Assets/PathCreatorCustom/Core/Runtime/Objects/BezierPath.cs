...
namespace PathCreation {
 ...
 IsClosed = isClosed;
        ...
        public Vector3 GetPoint (int i) {
            if (points == null || i < 0 || i >= points.Count) return Vector3.zero;
            return points[i];
        }
        ...
        public void AddSegmentToEnd (Vector3 anchorPos) {
            ...
            if (perAnchorNormalsAngle == null) perAnchorNormalsAngle = new List<float>();

            if (points.Count <= 0)
            {
                points.Add(anchorPos - Vector3.forward);
                perAnchorNormalsAngle.Add(0f);
                points.Add(anchorPos - Vector3.forward * 0.66f);
                points.Add(anchorPos - Vector3.forward * 0.33f);
