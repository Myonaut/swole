...
namespace PathCreationEditor
{
    public class ScreenSpacePolyLine
    {
        ...
        public MouseInfo CalculateMouseInfo()
        {
            ...
            float timeAlongBezierSegment = 0f;
            if (segmentStartIndices != null && segmentStartIndices.Length > 0 && closestBezierSegmentIndex + 1 < segmentStartIndices.Length)
            {
                ...
                timeAlongBezierSegment = bezierSegmentLength <= 0f ? 0f : (distanceAlongBezierSegment / bezierSegmentLength);
            }
