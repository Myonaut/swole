...
namespace Swole
{
    // Same functionality as Unity Editor's 'ColorMutator', which for whatever reason is not accessible at runtime.
    /// <summary>
    ...
    public class AdvancedColor
    {
        ...
        public AdvancedColor(Color originalColor)
        {
            m_OriginalColor = originalColor;
            Reset();
        }
        public static implicit operator AdvancedColor(Color originalColor) => new AdvancedColor(originalColor);
        public static implicit operator Color(AdvancedColor color) => color.exposureAdjustedColor;
        ...
        private static readonly AdvancedColor _globalColor = new AdvancedColor(Color.clear);
        public static AdvancedColor GlobalColor => _globalColor;
        public static void SetGlobalBaseColor(Color baseColor)
        {
            _globalColor.m_OriginalColor = baseColor;
            _globalColor.Reset();
        }
        public static void SetGlobalBaseHSVColor(Vector3 hsv, float alpha = 1f)
        {
            Color baseColor = Color.HSVToRGB(hsv.x, hsv.y, hsv.z, false);
            baseColor.a = alpha;
            SetGlobalBaseColor(baseColor);
        }
        public static void SetGlobalColorHDR(Color hdrColor)
        {
            DecomposeHdrColor(hdrColor, out Color32 baseLinearColor, out float exposure);

            SetGlobalBaseColor(baseLinearColor);
            _globalColor.exposureValue = exposure;
        }
        public static void SetGlobalHSVColorHDR(Vector3 hsv, float alpha = 1f)
        {
            Color baseColor = Color.HSVToRGB(hsv.x, hsv.y, hsv.z, true);
            baseColor.a = alpha;
            SetGlobalColorHDR(baseColor);
        }
        public static void SetGlobalColorExposure(float exposure)
        {
            _globalColor.exposureValue = exposure;
        }
        public static Color GetGlobalBaseColor() => _globalColor.color;
        public static Vector3 GetGlobalBaseColorHSV => _globalColor.colorHsv;
        public static Color GetGlobalColor() => _globalColor.exposureAdjustedColor;
        public static float GetGlobalColorExposure() => _globalColor.exposureValue;
    }
}