...
namespace Swole
{
    ...
    [Serializable]
    public struct CreationVariable
    {
        ...
        /// <summary>
        /// Used to convert a variable from one transform space to another (if applicable).
        /// </summary>
        ...
        /// <summary>
        /// This enum is used to describe a variable. The variable will be treated differently during space conversion depending on this type, even if the value type is identical. E.g. Type.PositionLocal will be treated as a point, whereas Type.DirectionLocal will be treated as a vector. Both are Vector3s.
        /// Note: the transform space suffix in this type's name (Local, Root, World), if present, cannot be enforced and may not be accurate. If the variable was created through the Swole API, then the suffix should be accurate and should represent the transform space after a conversion method is applied.
        /// </summary>
        public CreationVariable.Type type;
        ...
        /// <summary>
        /// Source code to drive the value of the variable. (Forces variable to be read-only if used.)
        /// </summary>
        public string driverScript;
        ...
        private void LogGetConversionError(SwoleLogger logger, Exception ex, System.Type conversionType)
        {
            if (logger != null)
            {
                logger.LogError($"Error convering variable '{name}' to {conversionType.Name}!");
        ...
        private void LogSetConversionError(SwoleLogger logger, Exception ex, System.Type conversionType)
        {
            if (logger != null)
            {
                logger.LogError($"Error setting variable '{name}' to {conversionType.Name}!");
                logger.LogError($"[{ex.GetType().ToString()}] {ex.Message}");
            }
        }
